...
CREATE TABLE wms_inbound_putaway_line (
    ...
    to_location_id BIGINT NOT NULL DEFAULT 0 COMMENT '目标库位(上架确认前为0)',
    batch_no VARCHAR(64),
    lot_attrs VARCHAR(1024),
    status VARCHAR(32) NOT NULL DEFAULT 'CREATED',
...
--
-- 说明: 各 CREATE TABLE 里已经声明了一批常用 KEY，这里只补充"尚未覆盖"的索引。
--       不要在此重复建同名/同列索引，也不要引用不存在的表名或列名 ——
--       本段位于脚本末尾，一旦报错会让整个 init.sql 以非 0 状态结束。
-- ============================================================

-- 库存: 按批次查询（uk_stock_key 的 batch_no 位于末位，无法单独命中）
CREATE INDEX idx_stock_batch ON wms_inventory_stock(batch_no);
...
-- 策略: 按类型 + 启用状态匹配
CREATE INDEX idx_strategy_type ON wms_strategy_config(strategy_type, is_enabled);
