...
/**
 * 仓库领域服务 —— 封装跨聚合根的领域逻辑、业务规则校验。
 * 不标注 @Transactional（事务由应用层管理）。
 */
@Service
...
public class WarehouseDomainService {
    ...
    public void validateCreate(Warehouse warehouse) {
        ...
        if (warehouseRepository.existsByCode(warehouse.getTenantId(), warehouse.getWhCode(), null)) {
    ...
    public void validateUpdate(Warehouse warehouse) {
        warehouse.validateCode();
        if (warehouseRepository.existsByCode(warehouse.getTenantId(), warehouse.getWhCode(), warehouse.getId())) {
            throw BusinessException.conflict("仓库编码 [" + warehouse.getWhCode() + "] 已存在");
        }
    }
    ...
    /**
     * 禁用仓库 —— 业务规则：确认无库存后可禁用
     * 此处仅做规则校验，具体库存检查由 Inventory 域通过事件实现。
     */
