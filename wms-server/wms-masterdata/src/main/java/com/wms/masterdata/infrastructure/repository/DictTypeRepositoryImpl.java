...
public class DictTypeRepositoryImpl implements DictTypeRepository {
    ...
    public List<DictType> findAll(Long tenantId) {
        return mapper.selectList(new LambdaQueryWrapper<DictType>()
    ...
    public Optional<DictType> findByCode(Long tenantId, String typeCode) {
        return Optional.ofNullable(mapper.selectOne(new LambdaQueryWrapper<DictType>()
                .eq(DictType::getTenantId, tenantId)
                .eq(DictType::getTypeCode, typeCode)));
    }

    @Override public void save(DictType dictType) { mapper.insert(dictType); }
    @Override public void update(DictType dictType) { mapper.updateById(dictType); }
