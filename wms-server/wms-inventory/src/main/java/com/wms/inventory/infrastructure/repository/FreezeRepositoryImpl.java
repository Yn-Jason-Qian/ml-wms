...
public class FreezeRepositoryImpl implements FreezeRepository {
    ...
    @Override
    public Optional<Freeze> findById(Long id) {
    ...
    public List<Freeze> findByStatus(Long tenantId, String status) {
        return mapper.selectList(
                new LambdaQueryWrapper<Freeze>()
                        .eq(Freeze::getTenantId, tenantId)
                        .eq(Freeze::getStatus, status));
    }
    ...
    public List<Freeze> findBySku(Long tenantId, Long skuId) {
        return mapper.selectList(
                new LambdaQueryWrapper<Freeze>()
                        .eq(Freeze::getTenantId, tenantId)
                        .eq(Freeze::getSkuId, skuId));
    }

    @Override
    public void save(Freeze freeze) {
        mapper.insert(freeze);
    }

    @Override
    public void update(Freeze freeze) {
        mapper.updateById(freeze);
    }
}
