...
public class StockDomainService {
    ...
    /** 库存移动：来源扣减 → 目标增加 → 两条流水 */
    public Stock moveStock(Stock fromStock, Stock toStock, BigDecimal qty,
Long refId, String refNo, Long moveBy) {
        // 来源扣减
        fromStock.deduct(qty);
        stockRepository.updateWithVersion(fromStock);
        writeTxn(fromStock, "MOVE", "OUT", qty, refNo, refId, null, toStock.getLocationId(), moveBy);
        ...
        writeTxn(toStock, "MOVE", "IN", qty, refNo, refId, fromStock.getLocationId(), toStock.getLocationId(), moveBy);
    ...
    public void freezeStock(Stock stock, Freeze freeze, Long userId) {
        ...
        writeTxn(stock, "FREEZE", "OUT", freeze.getFreezeQty(), freeze.getId().toString(),
                freeze.getId(), null, null, userId);
    }
    ...
    public void unfreezeStock(Stock stock, Freeze freeze, Long userId) {
        ...
        stockRepository.updateWithVersion(stock);
        writeTxn(stock, "UNFREEZE", "IN", freeze.getFreezeQty(), freeze.getId().toString(),
                freeze.getId(), null, null, userId);
    }

    /** 盘点调整库存 */
    public void adjustStock(Stock stock, BigDecimal diffQty, Long refId, String refNo, Long userId) {
    ...
    private void writeTxn(Stock stock, String txnType, String direction, BigDecimal qty,
String refNo, Long refId, Long fromLoc, Long toLoc, Long operator) {
