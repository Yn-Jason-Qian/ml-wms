...
Long tenantId = UserContext.getTenantId();
        ...
        LambdaQueryWrapper<Stock> qw = new LambdaQueryWrapper<Stock>()
                ...
                .eq(query.getWarehouseId() != null, Stock::getWarehouseId, query.getWarehouseId())
                .eq(query.getOwnerId() != null, Stock::getOwnerId, query.getOwnerId())
                .eq(query.getLocationId() != null, Stock::getLocationId, query.getLocationId())
    ...
    public StockDTO findStockById(Long id) {
        return assembler.toDTO(stockRepository.findById(id)
    ...
    public List<StockTransactionDTO> findTransactionsByStock(Long stockId) {
        return txnRepository.findByStockId(UserContext.getTenantId(), stockId)
    ...
    public IPage<StockTransactionDTO> pageTransactions(TransactionQuery query) {
        ...
        IPage<StockTransaction> result = txnMapper.selectPage(page, new LambdaQueryWrapper<StockTransaction>()
                .eq(StockTransaction::getTenantId, UserContext.getTenantId())
                .eq(query.getStockId() != null, StockTransaction::getStockId, query.getStockId())
                .eq(query.getSkuId() != null, StockTransaction::getSkuId, query.getSkuId())
                .eq(query.getRefId() != null, StockTransaction::getRefId, query.getRefId())
                .eq(query.getTxnType() != null, StockTransaction::getTxnType, query.getTxnType())
...
String moveNo = "MV-" + LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyyMMddHHmmss"));

        // 创建移库单头
        MoveHeader header = new MoveHeader();
        header.setTenantId(tenantId); header.setWarehouseId(cmd.getWarehouseId());
        header.setMoveNo(moveNo); header.setMoveType(cmd.getMoveType());
        header.setStatus(MoveHeader.STATUS_CREATED); header.setRemark(cmd.getRemark());
        header.setCreatedBy(userId); header.setUpdatedBy(userId);
        moveRepository.saveHeader(header);

        // 创建移库单行 + 执行移动
        Stock fromStock = stockRepository.findByKey(tenantId, cmd.getWarehouseId(),
                        cmd.getFromLocationId(), cmd.getSkuId(), cmd.getBatchNo())
                .orElseThrow(() -> BusinessException.notFound("来源库位库存不存在或不足"));
        Stock toStock = stockRepository.findByKey(tenantId, cmd.getWarehouseId(),
                cmd.getToLocationId(), cmd.getSkuId(), cmd.getBatchNo()).orElse(null);

        stockDomainService.moveStock(fromStock, toStock, cmd.getMoveQty(), header.getId(), moveNo, userId);

        MoveLine line = new MoveLine();
        line.setTenantId(tenantId); line.setMoveHeaderId(header.getId()); line.setLineNo(1);
        line.setSkuId(cmd.getSkuId()); line.setSkuCode(fromStock.getSkuCode());
        line.setSkuName(fromStock.getSkuName()); line.setMoveQty(cmd.getMoveQty());
        line.setFromLocationId(cmd.getFromLocationId()); line.setFromStockId(fromStock.getId());
        line.setToLocationId(cmd.getToLocationId()); line.setBatchNo(cmd.getBatchNo());
        line.setStatus("DONE"); line.setMoveBy(userId); line.setMoveAt(LocalDateTime.now());
        line.setCreatedBy(userId); line.setUpdatedBy(userId);
...
Long userId = UserContext.getUserId();
        ...
        Stock stock = stockRepository.findById(cmd.getStockId())
                .orElseThrow(() -> BusinessException.notFound("库存记录不存在"));

        Freeze freeze = new Freeze();
        freeze.setTenantId(tenantId); freeze.setWarehouseId(cmd.getWarehouseId());
        freeze.setFreezeType(cmd.getFreezeType()); freeze.setStockId(cmd.getStockId());
        freeze.setSkuId(cmd.getSkuId()); freeze.setLocationId(cmd.getLocationId());
        freeze.setBatchNo(cmd.getBatchNo()); freeze.setFreezeQty(cmd.getFreezeQty());
        freeze.setReason(cmd.getReason());
        freeze.setStatus(Freeze.STATUS_ACTIVE); freeze.setFreezeBy(userId);
        freeze.setFreezeAt(LocalDateTime.now());
        freeze.setCreatedBy(userId); freeze.setUpdatedBy(userId);
...
Long userId = UserContext.getUserId();
        Freeze freeze = freezeRepository.findById(freezeId)
        ...
        Stock stock = stockRepository.findById(freeze.getStockId())
                .orElseThrow(() -> BusinessException.notFound("库存记录不存在"));

        freeze.release(); freeze.setReleaseBy(userId);
...
String no = "ST-" + LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyyMMddHHmmss"));

        StocktakeHeader h = new StocktakeHeader();
        h.setTenantId(tenantId); h.setWarehouseId(cmd.getWarehouseId());
        h.setStocktakeNo(no); h.setStocktakeType(cmd.getStocktakeType());
        h.setStocktakeMode(cmd.getStocktakeMode()); h.setLocationRange(cmd.getLocationRange());
        h.setPlanStartTime(cmd.getPlanStartTime()); h.setPlanEndTime(cmd.getPlanEndTime());
        h.setStatus(StocktakeHeader.STATUS_CREATED); h.setTotalLines(0);
