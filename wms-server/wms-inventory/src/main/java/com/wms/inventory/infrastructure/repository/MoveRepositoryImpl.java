...
public class MoveRepositoryImpl implements MoveRepository {
    ...
    @Override
    public Optional<MoveHeader> findHeaderById(Long id) {
        return Optional.ofNullable(headerMapper.selectById(id));
    }

    @Override
    public void saveHeader(MoveHeader h) {
        headerMapper.insert(h);
    }

    @Override
    public void updateHeader(MoveHeader h) {
        headerMapper.updateById(h);
    }

    @Override
    public void saveLine(MoveLine l) {
        lineMapper.insert(l);
    }

    @Override
    public void updateLine(MoveLine l) {
    ...
    public List<MoveLine> findLinesByHeader(Long headerId) {
        return lineMapper.selectList(
                new LambdaQueryWrapper<MoveLine>()
                        .eq(MoveLine::getMoveHeaderId, headerId)
                        .orderByAsc(MoveLine::getLineNo));
    }

    @Override
    public Optional<MoveLine> findLineById(Long id) {
