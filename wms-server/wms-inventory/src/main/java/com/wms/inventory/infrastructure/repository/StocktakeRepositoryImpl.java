...
public class StocktakeRepositoryImpl implements StocktakeRepository {
    ...
    @Override
    public Optional<StocktakeHeader> findHeaderById(Long id) {
        return Optional.ofNullable(headerMapper.selectById(id));
    }

    @Override
    public void saveHeader(StocktakeHeader h) {
        headerMapper.insert(h);
    }

    @Override
    public void updateHeader(StocktakeHeader h) {
        headerMapper.updateById(h);
    }

    @Override
    public void saveLine(StocktakeLine l) {
        lineMapper.insert(l);
    }

    @Override
    public void updateLine(StocktakeLine l) {
    ...
    public List<StocktakeLine> findLinesByHeader(Long headerId) {
        return lineMapper.selectList(
                new LambdaQueryWrapper<StocktakeLine>()
                        .eq(StocktakeLine::getStocktakeHeaderId, headerId)
                        .orderByAsc(StocktakeLine::getLineNo));
    }

    @Override
    public Optional<StocktakeLine> findLineById(Long id) {
