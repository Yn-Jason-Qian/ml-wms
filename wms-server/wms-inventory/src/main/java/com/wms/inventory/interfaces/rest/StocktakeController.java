...
public class StocktakeController {
    ...
    @PostMapping("/page")
    public ApiResponse<PageResponse<Map<String, Object>>> page(@Valid @RequestBody StocktakePageQuery query) {
        IPage<StocktakeHeader> result = stocktakeHeaderMapper.selectPage(new Page<>(query.getPageNum(), query.getPageSize()),
                new LambdaQueryWrapper<StocktakeHeader>()
                        .eq(query.getWarehouseId() != null, StocktakeHeader::getWarehouseId, query.getWarehouseId())
                        .eq(query.getStatus() != null, StocktakeHeader::getStatus, query.getStatus())
                        .orderByDesc(StocktakeHeader::getCreatedAt));
        return ApiResponse.ok(PageResponse.of(result.getRecords().stream().map(h -> {
            java.util.Map<String, Object> m = new java.util.HashMap<>();
            m.put("id", h.getId()); m.put("stocktakeNo", h.getStocktakeNo()); m.put("warehouseId", h.getWarehouseId());
            m.put("stocktakeType", h.getStocktakeType()); m.put("stocktakeMode", h.getStocktakeMode());
            m.put("status", h.getStatus()); m.put("totalLines", h.getTotalLines()); m.put("diffLines", h.getDiffLines());
