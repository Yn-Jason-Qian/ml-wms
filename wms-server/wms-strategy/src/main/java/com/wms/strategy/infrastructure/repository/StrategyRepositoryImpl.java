...
public class StrategyRepositoryImpl implements StrategyRepository {
    ...
    @Override
    public Optional<StrategyConfig> findById(Long id) { return Optional.ofNullable(configMapper.selectById(id)); }

    @Override
    public Optional<StrategyConfig> findByCode(Long tenantId, String code) {
        return Optional.ofNullable(configMapper.selectOne(new LambdaQueryWrapper<StrategyConfig>()
    ...
    @Override public void saveConfig(StrategyConfig c) { configMapper.insert(c); }
    @Override public void updateConfig(StrategyConfig c) { configMapper.updateById(c); }
    @Override public void deleteConfig(Long id) { configMapper.deleteById(id); }

    @Override
    public List<StrategyRule> findRulesByStrategy(Long strategyId) {
        return ruleMapper.selectList(new LambdaQueryWrapper<StrategyRule>()
                .eq(StrategyRule::getStrategyId, strategyId)
                .eq(StrategyRule::getIsEnabled, 1)
                .orderByAsc(StrategyRule::getRuleNo));
    }

    @Override public Optional<StrategyRule> findRuleById(Long id) { return Optional.ofNullable(ruleMapper.selectById(id)); }
    @Override public void saveRule(StrategyRule r) { ruleMapper.insert(r); }
    @Override public void updateRule(StrategyRule r) { ruleMapper.updateById(r); }
    @Override public void deleteRule(Long id) { ruleMapper.deleteById(id); }

    @Override
    public boolean existsByCode(Long tenantId, String code, Long excludeId) {
Long count = configMapper.selectCount(new LambdaQueryWrapper<StrategyConfig>()
                .eq(StrategyConfig::getTenantId, tenantId)
                .eq(StrategyConfig::getStrategyCode, code)
