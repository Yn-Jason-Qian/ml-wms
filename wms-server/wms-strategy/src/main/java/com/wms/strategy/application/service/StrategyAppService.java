...
Long tenantId = UserContext.getTenantId();
        return strategyRepository.findByType(tenantId, strategyType).stream()
                .map(c -> {
                    List<StrategyRule> rules = strategyRepository.findRulesByStrategy(c.getId());
    ...
    public List<StrategyConfigDTO> listAll() {
        return strategyRepository.findAll(UserContext.getTenantId()).stream()
                .map(c -> assembler.toDTO(c, null)).collect(Collectors.toList());
    }

    public StrategyConfigDTO getById(Long id) {
        StrategyConfig c = strategyRepository.findById(id)
    ...
    public StrategyConfigDTO createConfig(StrategyConfigCreateCmd cmd) {
        ...
        c.setCreatedBy(UserContext.getUserId()); c.setUpdatedBy(UserContext.getUserId());
    ...
    @Transactional
    public void deleteConfig(Long id) { strategyRepository.deleteConfig(id); }
    ...
    public StrategyRuleDTO addRule(StrategyRuleCreateCmd cmd) {
        ...
        r.setCreatedBy(UserContext.getUserId()); r.setUpdatedBy(UserContext.getUserId());
    ...
    public void updateRule(Long id, StrategyRuleCreateCmd cmd) {
        StrategyRule r = strategyRepository.findRuleById(id)
                .orElseThrow(() -> BusinessException.notFound("规则不存在"));
        r.setRuleNo(cmd.getRuleNo()); r.setRuleName(cmd.getRuleName());
        r.setConditionsJson(cmd.getConditionsJson()); r.setActionsJson(cmd.getActionsJson());
        r.setUpdatedBy(UserContext.getUserId());
        strategyRepository.updateRule(r);
    }

    @Transactional
    public void deleteRule(Long id) { strategyRepository.deleteRule(id); }
...
Long tenantId = UserContext.getTenantId();
        ...
        StrategyEngine.MatchResult result = engine.match(request.getStrategyType(), context,
                type -> strategyRepository.findByType(tenantId, type).stream()
                        .peek(c -> c.setRules(strategyRepository.findRulesByStrategy(c.getId())))
                        .collect(Collectors.toList()));
