...
public class TaskController {
    ...
    public ApiResponse<PageResponse<Map<String, Object>>> page(@Valid @RequestBody TaskPageQuery query) {
        IPage<TaskHeader> result = taskHeaderMapper.selectPage(new Page<>(query.getPageNum(), query.getPageSize()),
                new LambdaQueryWrapper<TaskHeader>()
                        .eq(query.getWarehouseId() != null, TaskHeader::getWarehouseId, query.getWarehouseId())
                        .eq(query.getStatus() != null, TaskHeader::getStatus, query.getStatus())
                        .eq(query.getTaskType() != null, TaskHeader::getTaskType, query.getTaskType())
                        .orderByDesc(TaskHeader::getCreatedAt));
        return ApiResponse.ok(PageResponse.of(result.getRecords().stream().map(t -> {
            java.util.Map<String, Object> m = new java.util.HashMap<>();
            m.put("id", t.getId()); m.put("taskNo", t.getTaskNo()); m.put("taskType", t.getTaskType());
            m.put("warehouseId", t.getWarehouseId()); m.put("status", t.getStatus()); m.put("priority", t.getPriority());
            m.put("assignTo", t.getAssignTo() != null ? t.getAssignTo() : 0);
            m.put("taskSourceNo", t.getTaskSourceNo() != null ? t.getTaskSourceNo() : "");
