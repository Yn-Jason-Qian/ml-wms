...
String orderNo = "ORD-" + LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyyMMddHHmmss"));
        ...
        h.setOrderNo(orderNo); h.setOrderType(cmd.getOrderType()); h.setSourceNo(cmd.getSourceNo());
        h.setCustomerName(cmd.getCustomerName()); h.setCustomerAddress(cmd.getCustomerAddress());
        h.setExpectedShipTime(cmd.getExpectedShipTime());
        h.setPriority(cmd.getPriority() != null ? cmd.getPriority() : 5);
        h.setStatus(OrderHeader.STATUS_CREATED); h.setCreatedBy(userId); h.setUpdatedBy(userId);
        outboundRepo.saveOrder(h);

        int lineNo = 0;
        for (OrderCreateCmd.LineItem item : cmd.getLines()) {
            lineNo++;
            Sku sku = skuRepository.findById(item.getSkuId()).orElseThrow(() -> BusinessException.notFound("SKU不存在"));
            OrderLine l = new OrderLine();
            l.setTenantId(tenantId); l.setOrderHeaderId(h.getId()); l.setLineNo(lineNo);
            l.setSkuId(item.getSkuId()); l.setSkuCode(sku.getSkuCode()); l.setSkuName(sku.getSkuName());
            l.setOrderQty(item.getOrderQty()); l.setAllocatedQty(BigDecimal.ZERO);
            l.setPickedQty(BigDecimal.ZERO); l.setShippedQty(BigDecimal.ZERO);
...
Long tenantId = UserContext.getTenantId();Long userId = UserContext.getUserId();
String waveNo = "WAV-" + LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyyMMddHHmmss"));

        WaveHeader h = new WaveHeader();
        h.setTenantId(tenantId); h.setWarehouseId(cmd.getWarehouseId());
        h.setWaveNo(waveNo); h.setWaveType(cmd.getWaveType());
        h.setWaveStatus(WaveHeader.STATUS_CREATED); h.setOrderCount(cmd.getOrderIds().size());
        ...
        for (Long orderId : cmd.getOrderIds()) {
            ...
            wl.setTenantId(tenantId); wl.setWaveHeaderId(h.getId());
            wl.setOrderHeaderId(orderId); wl.setSortOrder(++sort);
            wl.setCreatedBy(userId); wl.setUpdatedBy(userId);
            outboundRepo.saveWaveLine(wl);

            // 更新订单关联波次
            OrderHeader order = outboundRepo.findOrderById(orderId).orElse(null);
            if (order != null) { order.setWaveHeaderId(h.getId()); outboundRepo.updateOrder(order); }
        }

        h.release(); h.setReleasedBy(userId);
...
Long tenantId = UserContext.getTenantId();Long userId = UserContext.getUserId();
String pickNo = "PICK-" + LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyyMMddHHmmss"));

        WaveHeader wave = outboundRepo.findWaveById(waveHeaderId)
                .orElseThrow(() -> BusinessException.notFound("波次不存在"));

        PickHeader h = new PickHeader();
        h.setTenantId(tenantId); h.setWarehouseId(wave.getWarehouseId());
        h.setPickNo(pickNo); h.setWaveHeaderId(waveHeaderId);
        h.setPickType("RF"); h.setStatus(PickHeader.STATUS_CREATED);
        h.setCreatedBy(userId); h.setUpdatedBy(userId);
        ...
        for (WaveLine wl : outboundRepo.findWaveLines(waveHeaderId)) {
            for (OrderLine ol : outboundRepo.findOrderLines(wl.getOrderHeaderId())) {
                ...
                pl.setTenantId(tenantId); pl.setPickHeaderId(h.getId()); pl.setLineNo(lineNo);
                pl.setOrderHeaderId(ol.getOrderHeaderId()); pl.setOrderLineId(ol.getId());
                pl.setSkuId(ol.getSkuId()); pl.setSkuCode(ol.getSkuCode()); pl.setSkuName(ol.getSkuName());
                pl.setPickQty(ol.getOrderQty()); pl.setPickedQty(BigDecimal.ZERO);
                pl.setBatchNo(ol.getBatchNo()); pl.setLotAttrs(ol.getLotAttrs());
...
Long userId = UserContext.getUserId();
        PickHeader h = outboundRepo.findPickById(cmd.getPickHeaderId())
                .orElseThrow(() -> BusinessException.notFound("拣货单不存在"));
        PickLine l = outboundRepo.findPickLines(h.getId()).stream()
                .filter(p -> p.getId().equals(cmd.getPickLineId())).findFirst()
                .orElseThrow(() -> BusinessException.notFound("拣货行不存在"));

        l.setPickedQty(cmd.getPickedQty()); l.setToContainer(cmd.getToContainer());
        l.setStatus("PICKED"); l.setPickBy(userId); l.setPickAt(LocalDateTime.now());
        l.setUpdatedBy(userId);
        outboundRepo.updatePickLine(l);

        boolean allDone = outboundRepo.findPickLines(h.getId()).stream().allMatch(p -> "PICKED".equals(p.getStatus()));
        if (allDone) { h.setStatus(PickHeader.STATUS_PICKED); outboundRepo.updatePickHeader(h); }
    }
...
Long tenantId = UserContext.getTenantId();Long userId = UserContext.getUserId();
String shipNo = "SHP-" + LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyyMMddHHmmss"));

        ShipHeader h = new ShipHeader();
        h.setTenantId(tenantId); h.setWarehouseId(cmd.getWarehouseId()); h.setOwnerId(cmd.getOwnerId());
        h.setShipNo(shipNo); h.setWaveHeaderId(cmd.getWaveHeaderId());
        h.setDeliveryMethod(cmd.getDeliveryMethod()); h.setCarrierName(cmd.getCarrierName());
        h.setTrackingNo(cmd.getTrackingNo()); h.setPackageCount(cmd.getPackageCount());
        h.setGrossWeight(cmd.getGrossWeight()); h.setVolume(cmd.getVolume());
        h.setStatus(ShipHeader.STATUS_DONE); h.setShipBy(userId); h.setShipAt(LocalDateTime.now());
        h.setCreatedBy(userId); h.setUpdatedBy(userId);
